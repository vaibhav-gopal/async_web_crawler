...
impl TokenSink for &mut LinkQueue {
    ...
    fn process_token(&mut self, token: Token, line_number: u64) -> TokenSinkResult<Self::Handle> {
        match token {
            //pattern match to find a `Tag` type that is the html `StartTag`
            //we use ref because we want a reference to bind to `tag` rather than moving ownership
            //we also use @ to set a condition which is our type of `Tag`
            //we check the start tag to be a link element inside the match arm instead
                ...
                if tag.name.as_ref() == "a" {
                    for attribute in tag.attrs.iter() {
                        if attribute.name.local.as_ref() == "href" {
                            ...
                            self.links.push(String::from_utf8_lossy(url_str).to_string());
                        }
                    }
                }
            },
...
fn main() {
    task::block_on(async {
        box_crawl(vec![Url::parse("https://www.rust-lang.org").unwrap()], 1, 2).await.unwrap();
    })
}